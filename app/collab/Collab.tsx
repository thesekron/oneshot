...
import { APP_NAME, cloneJSON, EVENT, toBrandedType } from "@oneshot/common";
import {
  IDLE_THRESHOLD,
  ACTIVE_THRESHOLD,
  UserIdleState,
...
import { decryptData } from "@oneshot/excalidraw/data/encryption";
import { getVisibleSceneBounds } from "@oneshot/element";
import { newElementWith } from "@oneshot/element";
import { isImageElement, isInitializedImageElement } from "@oneshot/element";
import { AbortError } from "@oneshot/excalidraw/errors";
import { t } from "@oneshot/excalidraw/i18n";
import { withBatchedUpdates } from "@oneshot/excalidraw/reactUtils";

import throttle from "lodash.throttle";
import { PureComponent } from "react";

import { bumpElementVersions } from "@oneshot/excalidraw/data/restore";

import type {
  ReconciledExcalidrawElement,
  RemoteExcalidrawElement,
} from "@oneshot/excalidraw/data/reconcile";
import type { ImportedDataState } from "@oneshot/excalidraw/data/types";
import type {
  ExcalidrawElement,
  FileId,
...
import type { Mutable, ValueOf } from "@oneshot/common/utility-types";
...
class Collab extends PureComponent<CollabProps, CollabState> {
  ...
  startCollaboration = async (
    ...
    try {
      // Replace "localhost" with the actual hostname so that cross-device
      // sessions (e.g. laptop → phone via local network IP) connect to the
      // correct WebSocket server instead of the phone's own localhost.
      const wsServerUrl = import.meta.env.VITE_APP_WS_SERVER_URL.replace(
        "localhost",
