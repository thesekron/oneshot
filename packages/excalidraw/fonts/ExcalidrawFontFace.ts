import { promiseTry, LOCAL_FONT_PROTOCOL } from "@oneshot/common";
...
export class ExcalidrawFontFace {
  ...
  private static readonly ASSETS_FALLBACK_URL = `https://esm.sh/${
    import.meta.env.PKG_NAME
      ...
      : "@oneshot/excalidraw" // fallback to the latest package version (i.e. for app)
  }/dist/prod/`;
  ...
  private static createUrls(uri: string): URL[] | DataURL[] {
    ...
    // absolute assets paths, which are found in tests and oneshot app build, won't work with base url, so we are stripping initial slash away
