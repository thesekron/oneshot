...
if (!command || command === "install") {
  ...
  console.error(
    "Usage: npx oneshot-app [install|start [--resume <roomId>]|sessions]",
  );
  process.exit(1);
}
...
async function runInstall() {
  ...
  if (
    ...
    try {
      if (agent === "claude-code") {
        await setupClaudeCodeSkill();
      }
      if (agent === "cursor") {
        await setupCursorSkill();
      }
      if (agent === "antigravity") {
        await setupAntigravitySkill();
      }
      spinner.stop(pc.green("Skill installed"));
    } catch (err) {
      spinner.stop(
        pc.yellow(`Could not install skill: ${(err as Error).message}`),
      );
    }
  }
  ...
  p.outro(
    [
      ...
      "",
      pc.dim(
        "  The agent will open your canvas and start the sync automatically.",
      ),
    ].join("\n"),
  );
}
  ...
  try {
    if (config.sync === "ably") {
      ...
      outputJSON({
        ...
        message: `Unknown sync type: ${config.sync}`,
      });
      process.exit(1);
    }
  } catch (err) {
